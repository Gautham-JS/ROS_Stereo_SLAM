...
class visualOdometry{
    ...
    public:
        ...
        visualOdometry(int Seq, const char*Lfptr, const char*Rfptr, std::string vocPath){
            lFptr = Lfptr;
            rFptr = Rfptr;
            vocfile = vocPath;
        ...
        void SORcloud(vector<Point3f>&pts3d, vector<Point3f>&colorMap){
            ...
            for(size_t i=0; i<pts3d.size(); i+=1){
                ...
                clPt.x = pts3d[i].x; clPt.y = pts3d[i].y; clPt.z = pts3d[i].z;
            ...
            pts3d.clear(); colorMap.clear();
            for(size_t i=0; i<bufferCloud->points.size(); i++){
                pcl::PointXYZRGB cldPt;
                cldPt = bufferCloud->points[i];
                pts3d[i].x = cldPt.x; pts3d[i].y = cldPt.y; pts3d[i].z = cldPt.z;
                colorMap[i].x = cldPt.b; colorMap[i].y = cldPt.g; colorMap[i].z = cldPt.r;
            }
        }
        ...
        void rosPublish(vector<vector<Point3f>>&pt3d, Mat&trajROS, Mat&Rmat){
            ...
            for(size_t k=0; k<pt3d.size(); k++){
                ...
                // for(pcl::PointXYZRGB&pts:bufferCloud->points){
                //     msg->points.emplace_back(pts);
                }
...
int main(int argc, char **argv){
    ...
    visualOdometry VO(0, impathL, impathR, vocPath);
    char FileName1[200], filename2[200];
